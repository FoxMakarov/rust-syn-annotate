...
impl<'a> Synthesizer<'a> {
    ...
    //为library中的component按顺序生成对应个数的立即数或者params或者output
    ...
    fn fresh_param_vars(&self, bit_width: u32) -> Vec<BitVec<'a>> {
        self.library
            ...
            .flat_map(|c| 
                {
                    //println!("operand_arity : {}", c.operand_arity());
                    (0..c.operand_arity()).map(|_| fresh_param(self.context, bit_width))
                }
            )
            .collect()
    }
    ...
    fn finite_synthesis(
        ...
        //按顺序生成library中的component需要的立即数构成的数组
        let immediates = self.fresh_immediates(bit_width);

        //println!("immediates : {}", immediates.len());
        let mut works_for_inputs = Vec::with_capacity(inputs.len() * 4);

        //按照生成inputs的组数来循环，这里默认是从4开始
        //inputs里面的每一组的个数是spec中var()的数量
        ...
        match solver.check() {
            ...
            z3::SatResult::Sat => {
                ...
                //println!("immediaties : {:?}", immediates);
                ...
                debug!(
                    ...
                    //用产生的位置信息去构造函数，对于库里面的component，按顺序第几个输入被第几行决定，等等
    ...
    //候补程序和待优化程序的非and在一起，然后扔给smt产生出结果就是反例，求不出来就是正确的
    ...
    fn connectivity(
        ...
        //println!("{}", self.locations.inputs.len());
        //代表spec中有几个var作为input

        //将locations中的Line和传入的值联系起来<Line, BitVec>大概这样
    ...
    pub fn synthesize(&mut self) -> Result<Program> {
        //产生几组Vec<u64>输入，目的是为了扩充初始的输入值，让smt求解器一下子能求很多组值，增加第一次生成的候选程序的正确性
    ...
    fn make_expression<'a>(
        ...
        for instr in self.instructions.iter().skip(inputs.len()) {
            ...
            instr
                ...
                .operands(|Id(x)| {
                //println!("x : {}", x);
                operands.push(vars[x as usize].clone())
